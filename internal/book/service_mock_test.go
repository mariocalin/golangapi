...
func (_m *MockBookService) CreateBook(command *BookCommand) (*Book, error) {
	...
	var r0 *Book
	var r1 error
	if rf, ok := ret.Get(0).(func(*BookCommand) (*Book, error)); ok {
		return rf(command)
	}
	if rf, ok := ret.Get(0).(func(*BookCommand) *Book); ok {
		r0 = rf(command)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*Book)
	...
	if rf, ok := ret.Get(1).(func(*BookCommand) error); ok {
		r1 = rf(command)
	} else {
		r1 = ret.Error(1)
	}
...
func (_m *MockBookService) UpdateBook(id *uuid.UUID, command *BookCommand) error {
	...
	var r0 error
	if rf, ok := ret.Get(0).(func(*uuid.UUID, *BookCommand) error); ok {
		r0 = rf(id, command)
	} else {
		r0 = ret.Error(0)
	}
